...
namespace glm
{
	...
	struct detail::compute_inverse<4, 4, float, Q, true>
	{
		GLM_FUNC_QUALIFIER static mat<4, 4, float, Q> call(mat<4, 4, float, Q> const &m)
		{
			...
			float32x4_t Vec2f = neon::copy_lane(neon::dupq_lane(m0, 2), 0, m1, 2);   // (m[1][2], m[0][2], m[0][2], m[0][2]);
			float32x4_t Vec3  = neon::copy_lane(neon::dupq_lane(m0, 3), 0, m1, 3);   // (m[1][3], m[0][3], m[0][3], m[0][3]);

			float32x4_t Inv0 = Vec1 * Fac0 - Vec2f * Fac1 + Vec3 * Fac2;
			float32x4_t Inv1 = Vec0 * Fac0 - Vec2f * Fac3 + Vec3 * Fac4;
			float32x4_t Inv2 = Vec0 * Fac1 - Vec1 * Fac3 + Vec3 * Fac5;
			float32x4_t Inv3 = Vec0 * Fac2 - Vec1 * Fac4 + Vec2f * Fac5;
