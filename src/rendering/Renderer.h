...
class Render
{
  private:
	...
	// std::unique_ptr<VertexBuffer> m_SimpleVertexBuffer;
	...
	void textImpl(std::string &text, float x, float y, float scale, glm::vec4 colour);
  ...
  public:
	...
	static void text(std::string &text, float x, float y, float scale, glm::vec4 colour) { get().textImpl(text, x, y, scale, colour); }
	...
	static std::array<Vertex, 4> createQuad(float x, float y, double rotation, float size, uint32_t texID, bool centering = true) { return get().createQuadImpl(x, y, rotation, size, size, texID, centering); }
